...
def _launcher_path() -> Path:
    """Return the stable executable facade used by detached workers."""
    launcher = Path(__file__).resolve().parents[1] / "codex_workflows.py"
    return launcher if launcher.is_file() else Path(__file__).resolve()
...
def _builtin_workflows_root() -> Path:
    # ``engine.py`` lives below ``skills/codex-workflows/scripts/workflow_runtime``.
    # Resolve the skill root from the package boundary so installed copies and
    # the repository launcher share one deterministic asset location.
    return Path(__file__).resolve().parents[2] / "assets" / "workflows"
...
def _mcp_root_auth_module():
    ...
    if existing is not None:
        return existing
    module_path = Path(__file__).resolve().parents[4] / "mcp" / "root_auth.py"
...
def _mcp_redaction_module():
    ...
    if existing is not None:
        return existing
    module_path = Path(__file__).resolve().parents[4] / "mcp" / "redaction.py"
    specification = importlib.util.spec_from_file_location(module_name, module_path)
    if specification is None or specification.loader is None:
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command.startswith("prompt-") or args.command == "_prompt_worker":
            ...
            return prompt_runtime.command(args, project, sys.modules[__name__])
